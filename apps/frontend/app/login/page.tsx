...
import { estUnDefi, type AuthStatus } from '@sentinel/shared-types';
...
function FormulaireConnexion() {
  ...
  /** Non nul dès que le mot de passe est validé et qu'un second facteur est réclamé. */
  ...
  const soumettre = async (evenement: React.FormEvent) => {
    ...
    setEnCours(true);
    try {
      const resultat = await api.auth.login({ username: username.trim(), password });
      if (estUnDefi(resultat)) {
        // Le mot de passe est bon, mais la session n'est pas ouverte : on passe
        // à la seconde étape sans quitter la page.
  ...
  const soumettreLeCode = async (evenement: React.FormEvent) => {
    evenement.preventDefault();
    setErreur(null);
    setEnCours(true);
    try {
      await api.twoFactor.challenge(defi!, code.trim());
      router.replace(suite);
    } catch (e) {
      ...
      <div className="w-full max-w-sm">
        ...
        {defi === null ? (
          ...
          <form
            ...
            className="space-y-4 rounded-lg border border-slate-200 bg-surface-raised p-5"
          >
            <div>
              <h1 className="text-sm font-medium text-slate-800">Vérification en deux étapes</h1>
            ...
            <div>
              ...
              <input
                ...
                // `one-time-code` permet au téléphone de proposer le code reçu ;
                // `inputMode` fait apparaître un pavé numérique.
                autoComplete="one-time-code"
                inputMode="numeric"
                autoFocus
                required
                placeholder="123456"
                className="w-full rounded border border-slate-300 px-3 py-2 text-center font-mono text-lg tracking-[0.3em] outline-none focus:border-sky-500"
              />
            </div>
            ...
            <div className="space-y-2 border-t border-slate-200 pt-3 text-xs text-slate-500">
              <p>
                Téléphone perdu ou inaccessible ? Saisissez l’un de vos <strong>codes de récupération</strong> dans le
                même champ.
              </p>
              <button type="button" onClick={recommencer} className="text-sky-700 hover:text-sky-900">
                ← Recommencer avec un autre compte
              </button>
            </div>
          </form>
        )}
